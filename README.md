...
- XGB Classifier (Final model with Hyperparamter tuning)

Hyperparameter tuning is performed using **RandomizedSearchCV** with recall to handle class imbalance.
...
```bash
python freight_cost_prediction/predict_cost.py
python invoice_flagging/invoice_flagging.py
```
3. Test Models:
```bash
python inference/predict_single_freight_cost.py
python inference/redict_single_invoice_flag.py
``` 
4. Open Application:
```bash
...
📧 Email: shreyparekh016@gmail.com  
🔗 [LinkedIn](https://www.linkedin.com/in/shrey-parekh-profile/)
